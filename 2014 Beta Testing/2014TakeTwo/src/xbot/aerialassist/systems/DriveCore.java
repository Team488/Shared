...
public class DriveCore extends Loggable {
    ...
    public static double remapMinusDeadband(double input, double deadband) {
        if(input > 0) {
            input -= deadband;
        } else {
            input += deadband;
        }
        input = input / (1 - deadband);
        
        // try squaring the input to make it ease into the low end
    ...
    {
        // massage the heading if invalid
        heading = heading % 360;
        if (heading < 0)
        {
            heading += 360;
